...
gnu:   # BUILDTARGET GNU Fortran, C, and C++ compilers
	...
	"FFLAGS_OPT = -g -std=f2008 -fallow-argument-mismatch -ffree-line-length-none -fconvert=big-endian -ffree-form -fbacktrace -fcheck=bounds -ffpe-trap=invalid,zero,overflow -fcheck=all" \
	"CFLAGS_OPT = -g -O3 -std=gnu89" \
	"CXXFLAGS_OPT = -O3" \
	"LDFLAGS_OPT = -O3" \
...
gnu-jaci:   # BUILDTARGET GNU Fortran, C, and C++ compilers
	( $(MAKE) all \
	"FC_PARALLEL = ftn" \
	"CC_PARALLEL = ftn" \
	"CXX_PARALLEL = ftn" \
...
ifneq ($(wildcard $(PNETCDF)/lib64), )
	...
	LIBS += -L$(PNETCDF)/$(PNETCDFLIBLOC) -lpnetcdf -lnetcdff -lnetcdf
endif
